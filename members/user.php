...
if($user_level=='2')
{
 ...
 //TAB BARCHE
      $sql = "SELECT b.* FROM cv_barche as b where b.id_utente = " .$uid;
      ...
      if(mysqli_num_rows($result) != 0) 
      {
         ...
         }else
         {
              ...
              /* SE non presente nella tabella rmatore*/
               $sql = "INSERT INTO cv_barche (id_utente,nome,cognome) SELECT id_utente,name as nome,surname as cognome FROM cv_generale where id_utente=" .$uid;
               $result = $mysqli->query($sql);
               $arr = array();
                if(mysqli_num_rows($result) != 0)
                {
      ...
      //TAB FORMAZIONE
          $sql = "SELECT * from oltremare.cv_formazione_oltremare WHERE id_utente =" .$uid ;
          $result = $mysqli->query($sql);
            $arr = array();
            if(mysqli_num_rows($result) != 0) 
            {
          ...
          //TAB FORMAZIONE
          $sql = "SELECT * from cv_recapiti  WHERE id_utente =" .$uid ." LIMIT 1";
          $result = $mysqli->query($sql);
            $arr = array();
            if(mysqli_num_rows($result) != 0) 
            {
              ...
              unlink("sezione/recapiti/recapiti.json");
              $myfile = fopen("sezione/formazione/recapiti.json", "w") or die("Unable to open file!");
              $txt =json_encode($arr);
              fwrite($myfile, $txt);
              fclose($myfile);
           }            
