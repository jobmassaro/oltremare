...
list($meta_title, $meta_description, $site_title, $site_email, $site_logo) = all_settings();
  ...
  $sql = "SELECT * from cv_prenotazione ORDER BY indata ASC";
...
<?php include('assets/comp/top-nav.php');?>
	<div id="wrapper" ng-controller="PrenotatiCtrl">
		<!-- Side Wrapper -->
        <div id="side-wrapper">
            <ul class="side-nav">
...
<p></p>
     ...
     <div ng-include src="'sezione/prenotacorso/templates/modificaPrenotazione.html'"></div>
    ...
    <tr ng-repeat="detail in details| filter:search_query" class="well well-sm" id="generale">
        ...
        <td >{{detail.email}}</td>
        <td >{{detail.corso_prenotato}}</td>
        <td >{{detail.periodo_prenotato}}</td>

        <td><button class="btn btn-warning" ng-click="modificaPrenotazione(detail)" title="Edit"><span class="glyphicon glyphicon-edit"></span></button></td>
        <td><button class="btn btn-danger" ng-click="cancellazionePrenotazione(detail)" title="Delete"><span class="glyphicon glyphicon-trash"></span></button></td>
    </tr>
    </table>
</div>
</div>
...
var app = angular.module('prenotati',['ngMaterial']);
...
<script src="sezione/prenotacorso/prenotati.js"></script>	
